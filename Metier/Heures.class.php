...
{	
	...
	public static function getBySem($id_prof,$sem,$annee_univ,$htype)
	{
		...
		if($sem=='s1')
		{
			...
			$req='select m.id,libelle as mois,IFNULL(t.annee,IF(m.id IN ('.$mois1.'),'.$annee1.','.$annee2.')) as annee,IFNULL(t.lundi,0) as lundi,IFNULL(t.mardi,0) as mardi,IFNULL(t.mercredi,0) as mercredi,IFNULL(t.jeudi,0) as jeudi,IFNULL(t.vendredi,0) as vendredi,IFNULL(t.samedi,0) as samedi,IFNULL(t.dimanche,0) as dimanche,IFNULL(t.total_mois,0) as total from mois m LEFT OUTER JOIN (select extract(MONTH from date_jour) as mois,extract(YEAR from date_jour) as annee,id_prof,sum(IF(DAYOFWEEK(date_jour)=2,nbre_heure,0)) as Lundi,sum(IF(DAYOFWEEK(date_jour)=3,nbre_heure,0)) as Mardi,sum(IF(DAYOFWEEK(date_jour)=4,nbre_heure,0)) as Mercredi,sum(IF(DAYOFWEEK(date_jour)=5,nbre_heure,0)) as Jeudi,sum(IF(DAYOFWEEK(date_jour)=6,nbre_heure,0)) as Vendredi,sum(IF(DAYOFWEEK(date_jour)=7,nbre_heure,0)) as Samedi,sum(IF(DAYOFWEEK(date_jour)=1,nbre_heure,0)) as Dimanche, sum(nbre_heure) as total_mois from heures where id_prof='.$id_prof.' and type=\''.$htype.'\' and ((extract(YEAR FROM date_jour)='.$annee1.' and extract(MONTH FROM date_jour) IN ('.$mois1.')) or (extract(YEAR FROM date_jour)='.$annee2.' and extract(MONTH FROM date_jour) IN ('.$mois2.'))) group by 1,2 order by 2,1) t ON m.id=t.mois where m.id in ('.$mois3.') order by 3,1';
		}
		else	
		{
			$mois='2,3,4,5,6,7';
			$req='select m.id,libelle as mois,IFNULL(t.annee,'.$annee2.') as annee,IFNULL(t.lundi,0) as lundi,IFNULL(t.mardi,0) as mardi,IFNULL(t.mercredi,0) as mercredi,IFNULL(t.jeudi,0) as jeudi,IFNULL(t.vendredi,0) as vendredi,IFNULL(t.samedi,0) as samedi,IFNULL(t.dimanche,0) as dimanche,IFNULL(t.total_mois,0) as total from mois m LEFT OUTER JOIN (select extract(MONTH from date_jour) as mois,extract(YEAR from date_jour) as annee,id_prof,sum(IF(DAYOFWEEK(date_jour)=2,nbre_heure,0)) as Lundi,sum(IF(DAYOFWEEK(date_jour)=3,nbre_heure,0)) as Mardi,sum(IF(DAYOFWEEK(date_jour)=4,nbre_heure,0)) as Mercredi,sum(IF(DAYOFWEEK(date_jour)=5,nbre_heure,0)) as Jeudi,sum(IF(DAYOFWEEK(date_jour)=6,nbre_heure,0)) as Vendredi,sum(IF(DAYOFWEEK(date_jour)=7,nbre_heure,0)) as Samedi,sum(IF(DAYOFWEEK(date_jour)=1,nbre_heure,0)) as Dimanche, sum(nbre_heure) as total_mois from heures where id_prof='.$id_prof.' and type=\''.$htype.'\' and (extract(YEAR FROM date_jour)='.$annee2.' and extract(MONTH FROM date_jour) IN ('.$mois.')) group by 1,2 order by 2,1) t ON m.id=t.mois where m.id in ('.$mois.') order by 3,1';
		}
